 ...
 */
console.log(
  `The test cases will not work for codesandbox
  So Please ensure you are submitting it on github
  and then submit it to the cp.masaischool.com platform
`
);
if (jest.setTimeout) jest.setTimeout(60000);
beforeAll(() => {
  // attempt is 1
  global.score = 1;
...
test("Restaurant Components exist", () => {
  ...
  restaurants.forEach((restaurant, i) => {
    const { name, type, rating, votes, price } = RestaurantRowElements(
      restaurant
    );
...
test("Restaurant page works correctly 1", async () => {
  ...
  const { name, type, rating, votes, price, image } = getRestaurantPageElements(
    container
  );
